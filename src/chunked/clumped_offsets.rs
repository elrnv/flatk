...
impl<O: AsRef<[usize]> + Set> GetOffset for ClumpedOffsets<O> {
    ...
    unsafe fn offset_value_unchecked(&self, index: usize) -> usize {
        let chunk_offsets = self.chunk_offsets.as_ref();
        let offsets = self.offsets.as_ref();
        debug_assert!(!chunk_offsets.is_empty());
        debug_assert!(!offsets.is_empty());
        // The following is safe by construction of ClumpedOffsets.
        let first = *chunk_offsets.get_unchecked(0);
        let offset = match chunk_offsets.binary_search(&(index + first)) {
            Ok(clump_idx) => *offsets.get_unchecked(clump_idx),
            Err(clump_idx) => {
                // Offset is in the middle of a clump.
                // Given that idx is not out of bounds as defined in the doc,
                // the following are safe because idx + first >= first.
                // so clump_idx > 0. The inequality is strict because binary_search failed.
                let begin_off = *offsets.get_unchecked(clump_idx - 1);
                let clump_dist = *offsets.get_unchecked(clump_idx) - begin_off;
                let begin_clump_off = *chunk_offsets.get_unchecked(clump_idx - 1);
                let clump_size = *chunk_offsets.get_unchecked(clump_idx) - begin_clump_off;
...
impl<O: AsRef<[usize]>> ClumpedOffsets<O> {
    ...
    pub fn sizes(&self) -> UnclumpedSizes {
        debug_assert!(!self.chunk_offsets.as_ref().is_empty());
        UnclumpedSizes {
            cur: unsafe { *self.chunk_offsets.as_ref().get_unchecked(0) },
    ...
    #[inline]
    pub fn iter(&self) -> UnclumpedOffsets {
        debug_assert!(!self.offsets.as_ref().is_empty());
        unsafe {
            UnclumpedOffsets {
                cur_stride: 0,
                cur_offset: *self.offsets.as_ref().get_unchecked(0),
...
impl<'a> Iterator for UnclumpedSizes<'a> {
    ...
    fn next(&mut self) -> Option<usize> {
        ...
        let clump_offset = unsafe { *chunk_offsets.get_unchecked(0) };
        let next_clump_offset = unsafe { *chunk_offsets.get_unchecked(1) };

        let clump_dist = next_offset - offset;
        let clump_size = next_clump_offset - clump_offset;
        let stride = clump_dist / clump_size;

        // Incrementing the head offset effectively pops the unclumped offset.
        *cur += 1;
        if *cur == next_clump_offset {
            // Pop the last internal offset, which also changes the stride.
            self.clumped_offsets.offsets = &offsets[1..];
            self.clumped_offsets.chunk_offsets = &chunk_offsets[1..];
        }
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        let n = self.clumped_offsets.len();
        (n, Some(n))
    }

    #[inline]
    fn count(self) -> usize {
        self.clumped_offsets.len()
    }
}
...
impl<'a> IntoSizes for ClumpedOffsets<&'a [usize]> {
    ...
    fn into_sizes(self) -> UnclumpedSizes<'a> {
        ...
        UnclumpedSizes {
            cur: unsafe { *self.chunk_offsets.get_unchecked(0) },
...
impl<'a> Iterator for UnclumpedOffsets<'a> {
    type Item = usize;

    #[inline]
    fn next(&mut self) -> Option<usize> {
        let UnclumpedOffsets {
            cur_stride,
            cur_offset,
            chunk_offsets,
            offsets,
        ...
        // The following is safe because we checked the length of chunk_offsets above.
        // Also the API enforces the size correspondence between
        // chunk_offsets and offsets. We also do a debug check above.
        let offset = unsafe { *offsets.get_unchecked(0) };
        let clump_offset = unsafe { *chunk_offsets.get_unchecked(0) };
        ...
        if *cur_offset == offset {
            ...
            if let Some(next_offset) = self.offsets.get(0) {
                ...
                let next_clump_offset = unsafe { *self.chunk_offsets.get_unchecked(0) };
...
impl<'a> SplitOffsetsAt for ClumpedOffsets<&'a [usize]> {
    ...
    /// `mid` is shared between the two outputs. In addition, return the
    /// offset of the middle element.
    ///
    /// # Panics
    ///
    ...
    fn split_offsets_at(
        ...
        assert!(mid < self.len());
        // This is safe since self.chunk_offsets is non-empty.
        let first = unsafe { *self.chunk_offsets.get_unchecked(0) };

        // Try to find the mid in our clumped offsets.
        // Note that it is the responsibility of the container to ensure that the split is valid.
        let mid_idx = self
            .chunk_offsets
            .binary_search(&(mid + first))
            .expect("Cannot split clumped offsets through a clump");
        let los = &self.offsets[..=mid_idx];
        let ros = &self.offsets[mid_idx..];
        // This is safe since both `los` and `ros` are non-empty
        // (since the index ranges ..=mid_idx and mid_idx.. are inclusive)
        let off = unsafe { *ros.get_unchecked(0) - *los.get_unchecked(0) };
        (
            ClumpedOffsets {
                chunk_offsets: &self.chunk_offsets[..=mid_idx],
                offsets: los,
            },
            ClumpedOffsets {
                chunk_offsets: &self.chunk_offsets[mid_idx..],
...
impl<O: AsRef<[usize]> + Set> IndexRange for ClumpedOffsets<O> {
...
impl<'a, O: AsMut<[usize]>> ViewMut<'a> for ClumpedOffsets<O> {
    ...
    fn view_mut(&'a mut self) -> Self::Type {
        ClumpedOffsets {
            chunk_offsets: self.chunk_offsets.as_mut(),
            offsets: self.offsets.as_mut(),
        }
    }
}
...
impl<O: AsRef<[usize]>> ClumpedOffsets<O> {
    ...
    pub fn new(chunk_offsets: O, offsets: O) -> Self {
        assert!(!chunk_offsets.as_ref().is_empty());
...
impl<'a, O: AsRef<[usize]> + Set> GetIndex<'a, ClumpedOffsets<O>> for usize {
    type Output = usize;
    #[inline]
...
impl<O: AsRef<[usize]> + Set> IsolateIndex<ClumpedOffsets<O>> for usize {
    type Output = usize;
    #[inline]
...
impl<O: RemovePrefix> RemovePrefix for ClumpedOffsets<O> {
    #[inline]
...
mod tests {
    ...
    fn split_offset_at_test() {
        ...
        // Test in the middle
        let (l, r, off) = clumped_offsets.view().split_offsets_at(4);
        ...
        let (l, r, off) = clumped_offsets.view().split_offsets_at(0);
        dbg!(&l);
        ...
        // Test at the end
        let (l, r, off) = clumped_offsets.view().split_offsets_at(12);
        assert_eq!(
